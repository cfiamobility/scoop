...
/**
 * Presenter for viewing a profile like.
...
public class ProfileLikesPresenter extends ProfileCommentPresenter implements
    ...
    private ProfileLikes getItemByIndex(int i) {
        if (mDataCache == null)
            return null;
    ...
    public void loadDataFromDatabase(String userId) {
        mProfileLikesInteractor.getProfileLikess(userId);
    }

    @Override
    public void setData(JSONArray postsResponse, JSONArray imagesResponse) {
        mDataCache = PostDataCache.createWithType(ProfileLikes.class);
        ...
        for (int i=0; i<postsResponse.length(); i++) {
            ...
            ProfileLikes profilePost = new ProfileLikes(jsonPost, jsonImage);
            mDataCache.getProfileLikesList().add(profilePost);
        }
    ...
    public void onBindViewHolderAtPosition(ProfileLikesContract.View.ViewHolder viewHolderInterface, int i) {
        ProfileLikes profilePost = getItemByIndex(i);
        bindProfileLikesDataToViewHolder(viewHolderInterface, profilePost);
    }

    public static void bindProfileLikesDataToViewHolder(
            ProfileLikesContract.View.ViewHolder viewHolderInterface, ProfileLikes profilePost) {
